...
```bash
# Scan first 100 tasks (default: no date filter, pageSize=100)
python 03-skills/beam-debug-issue-tasks/scripts/debug_issue_tasks.py <agent_id>

# Scan first 100 tasks with summary
python 03-skills/beam-debug-issue-tasks/scripts/debug_issue_tasks.py <agent_id> --summary

# Filter by date range (last N days)
python 03-skills/beam-debug-issue-tasks/scripts/debug_issue_tasks.py <agent_id> --days 7
...
| `--workspace`, `-w` | Workspace: bid or prod | prod |
| `--days`, `-d` | Look back period (1, 3, 7, 14, 30) | None (scan first N) |
| `--scan-first` | Scan first N tasks (no date filter) | 100 |
| `--page-size` | Page size for API calls | 100 |
| `--task-id`, `-t` | Debug specific task ID | - |
| `--summary`, `-s` | Show grouped summary | false |
| `--limit`, `-l` | Max tasks to show details for | 10 |
