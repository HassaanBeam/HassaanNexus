...
def fetch_all_tasks(
    ...
    while page <= max_pages:
        result = list_agent_tasks(client, agent_id, start_date, end_date, limit=page_size, page=page)
...
def print_summary(
    ...
    print(f"\n{'='*60}")
    if isinstance(days_label, int):
        print(f"TASK SUMMARY (Last {days_label} day{'s' if days_label > 1 else ''})")
    else:
        print(f"TASK SUMMARY ({days_label} tasks)")
    print(f"{'='*60}")
...
Environment Variables (in .env):
    ...
    parser.add_argument("--days", "-d", type=int, default=None, choices=[1, 3, 7, 14, 30],
                        help="Look back N days (default: no filter, scan first 100)")
    ...
    parser.add_argument("--scan-first", type=int, default=100,
                        help="Scan first N tasks without date filter (default: 100)")
    ...
    try:
        ...
        # Calculate date range (only if --days specified)
        start_str = None
        end_str = None
        if args.days:
        ...
        if args.days:
            ...
            all_tasks = fetch_all_tasks(beam, args.agent_id, start_str, end_str, page_size=args.page_size)
            print(f"Found {len(all_tasks)} total tasks in date range")
        else:
            # No date filter - scan first N tasks
            print(f"Scanning first {args.scan_first} tasks (no date filter, pageSize={args.page_size})...")
            all_tasks = fetch_all_tasks(beam, args.agent_id, page_size=args.page_size, max_pages=1)
            all_tasks = all_tasks[:args.scan_first]  # Limit to scan_first
            print(f"Scanned {len(all_tasks)} tasks")
        ...
        if args.summary:
            ...
            days_label = args.days if args.days else f"first {args.scan_first}"
            print_summary(all_tasks, issue_tasks, by_status, by_day, days_label)
        ...
        if issue_tasks:
            ...
            if args.days:
                ...
                print(f"\nNo issue tasks found in the first {args.scan_first} tasks")
